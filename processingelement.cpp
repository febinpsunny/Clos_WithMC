...
TFlit TProcessingElement::nextFlit()
{
	...
	if(packet.float_flag==1)
	{
		//cout<<"Flit with float flag generated"<<endl;
		flit.floatFlag=1;
	}
	else
		flit.floatFlag=0;
...
bool TProcessingElement::canShot(TPacket& packet)
{
	...
	if (TGlobalParams::traffic_distribution != TRAFFIC_TABLE_BASED)
	{
		...
		if (TGlobalParams::real_traffic == 1)
		{
			...
			if(shot>=1)
			{
				//cout<<"Returned shot value"<<shot<<endl;
...
TPacket TProcessingElement::benchmarktraffic(int shot)
{
	//cout<< "benchmarktraffic function accessed with shot value:"<<shot<<endl;
	TPacket p;
	p.src_id = local_id;
	p.src_hub_id = hub_id;
	p.src_cluster_id = cluster_id;
	...
	if(shot>1)
	{
		//cout<<"Packet float flag set as 1"<<endl;
		p.float_flag =1;
	}
	else
		p.float_flag =0;
