...
unsigned int TGlobalStats::getReceivedFloatPackets()
{
	unsigned int n = 0;
	...
	for (int y=0; y<TGlobalParams::mesh_dim_y; y++)
		for (int x=0; x<TGlobalParams::mesh_dim_x; x++)
			for(int p=0; p<PE_PER_HUB; p++)
				for(int h=0; h< HUBS_PER_CLUSTER; h++)
				{
					n += noc->git[x][y]->pe[p][h]->stats.getReceivedFloatPackets();
					//n += noc->git[x][y]->gwi->stats.getReceivedPackets();
				}
...
double  TGlobalStats::getEnergyPerBit()
{
	...
	return (((MOD_ENRG_PERBIT*(getReceivedPackets()-getReceivedFloatPackets())* PACKET_SIZE) + (MOD_ENRG_PERBIT*0.5*getReceivedFloatPackets()*44)+(MOD_ENRG_PERBIT*0.5*getReceivedFloatPackets()*(PACKET_SIZE-44)))/(getAverageThroughput() * PACKET_SIZE));
}
...
void TGlobalStats::showStats(std::ostream& out, bool detailed)
{
	...
	out << "% Total received Float packets: " << getReceivedFloatPackets() << endl;
